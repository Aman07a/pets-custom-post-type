...
<div class="container container--narrow page-section">
    ...
    if (isset($_GET["favcolor"])) {
        $petArgs["favColor"] = sanitize_text_field($_GET["favcolor"]);
    }
    if (isset($_GET["species"])) {
        $petArgs["species"] = sanitize_text_field($_GET["species"]);
    }
    if (isset($_GET["minyear"])) {
        $petArgs["minYear"] = sanitize_text_field($_GET["minyear"]);
    }
    if (isset($_GET["maxyear"])) {
        $petArgs["maxYear"] = sanitize_text_field($_GET["maxyear"]);
    }
    if (isset($_GET["minweight"])) {
        $petArgs["minWeight"] = sanitize_text_field($_GET["minweight"]);
    }
    if (isset($_GET["maxweight"])) {
        $petArgs["maxWeight"] = sanitize_text_field($_GET["maxweight"]);
    }
    if (isset($_GET["favhobby"])) {
        $petArgs["favHobby"] = sanitize_text_field($_GET["favhobby"]);
    }
    if (isset($_GET["favfood"])) {
        $petArgs["favFood"] = sanitize_text_field($_GET["favfood"]);
    }
    ...
    <p>
        This page took
        <strong><?php echo timer_stop(); ?></strong> seconds to prepare.
        Found
        <strong><?php echo number_format($allPets->found_posts); ?></strong>
        results (showing the first
        <?php echo $allPets->post_count; ?>).
    </p>
    ...
    <p>
        <br>
        Looking for an example query link?
        <a href="<?php echo site_url("/pet-adoption/?species=cat&favhobby=destroying%20stuff"); ?>">
            How about only cats whose favorite hobby is destroying stuff?
        </a>
    </p>
</div>
